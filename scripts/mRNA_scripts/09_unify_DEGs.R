...
compare_DEGs <- function(limma_file, deseq2_file, output_file) {
  ...
  # Return the merged data for further analysis
  return(merged_data)
}
...
main <- function() {
  ...
  comparisons <- list(
    list(limma_file="DEGs_LIMMA_B_vs_C.csv", deseq2_file="DEGs_DESeq2_B_vs_C.csv", output_meta="meta_B_vs_C.csv", comparison="Bacterial vs Control"),
    list(limma_file="DEGs_LIMMA_V_vs_C.csv", deseq2_file="DEGs_DESeq2_V_vs_C.csv", output_meta="meta_V_vs_C.csv", comparison="Viral vs Control"),
    list(limma_file="DEGs_LIMMA_B_vs_V.csv", deseq2_file="DEGs_DESeq2_B_vs_V.csv", output_meta="meta_B_vs_V.csv", comparison="Bacterial vs Viral")
  )
  
  # Loop through each comparison and run the analysis
  for (comp in comparisons) {
    # Compare LIMMA and DESeq2 results and get merged data
    merged_data <- compare_DEGs(comp$limma_file, comp$deseq2_file, comp$output_overlap)
    
    # Perform Fisher's meta-analysis on the merged data
    perform_meta_analysis(comp$output_meta)
  }
}
